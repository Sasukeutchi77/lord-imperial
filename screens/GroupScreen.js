...
import { createGroup, searchUserByUsername } from '../services/chat';
...
export default function GroupScreen({ navigation }) {
  ...
  const memberCountLabel = useMemo(
    () => `${members.length} membre${members.length > 1 ? 's' : ''} sélectionné${members.length > 1 ? 's' : ''}`,
    [members.length]
  );
  ...
  const lookupMember = async () => {
    try {
      ...
      if (user.uid === profile.uid) {
        Alert.alert('Info', 'Vous êtes déjà administrateur du groupe.');
        setCandidate(null);
        return;
      }
      setCandidate(user);
  ...
  const addMember = () => {
    ...
    if (members.some((item) => item.uid === candidate.uid)) {
      Alert.alert('Déjà ajouté', 'Cet utilisateur fait déjà partie de la sélection.');
      return;
    }
  ...
  const handleCreate = async () => {
    ...
    try {
      setLoading(true);
      // Passer localAvatarUri — createGroup s'occupe de l'upload via uploadConversationAvatar
      ...
      Alert.alert('Création impossible', error.message || 'Le groupe n\'a pas pu être créé.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <ScreenContainer withKeyboard>
      <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.subtitle}>
          Ajoutez une identité claire à votre groupe, puis invitez des membres via leur pseudo exact.
        </Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.avatarSection}>
            <Pressable onPress={pickAvatar} disabled={pickingImage} style={styles.avatarPressable}>
              <Animated.View style={{ transform: [{ scale: avatarScale }] }}>
                <Avatar uri={localAvatarUri} label={title || 'GR'} size={80} />
            ...
            <View style={styles.avatarInfo}>
              <Text style={styles.avatarLabel}>Photo du groupe</Text>
          ...
          <ThemedTextInput
            label="Nom du groupe"
            placeholder="Conseil Impérial"
            value={title}
            onChangeText={setTitle}
          />
          <ThemedTextInput
            label="Description"
            placeholder="Discussion interne, annonces, coordination…"
            value={description}
          ...
          />
          <PrimaryButton title="Chercher ce membre" onPress={lookupMember} loading={searching} />
        </View>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.section}>Membres sélectionnés</Text>
        ...
        <View style={styles.chipsWrap}>
          {members.length ? (
            ...
            <Text style={styles.emptyHint}>
              Vous pouvez créer le groupe maintenant et ajouter des membres plus tard.
            </Text>
          )}
        </View>

        <PrimaryButton title="Créer le groupe" onPress={handleCreate} loading={loading} />
      </ScrollView>
    </ScreenContainer>
  );
}
